...
public class Track10 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.track_10);

        androidId = Settings.Secure.getString(getContentResolver(), Settings.Secure.ANDROID_ID);

        // initialize map
        rootRef = FirebaseDatabase.getInstance().getReference();
        playersRef = rootRef.child("Track").child(androidId).child("October");
    ...
    public void saveMap(Map<String, Boolean> inputMap) {
        SharedPreferences pSharedPref = getApplicationContext().getSharedPreferences("progress_10", Context.MODE_PRIVATE);
        if (pSharedPref != null) {
            JSONObject jsonObject = new JSONObject(inputMap);
            String jsonString = jsonObject.toString();
            SharedPreferences.Editor editor = pSharedPref.edit();
            editor.remove("Track_10").commit();
            editor.putString("Track_10", jsonString);
            editor.commit();
        }
    }
    ...
    public void loadMap() {
        ...
        try {
            if (pSharedPref != null) {
                String jsonString = pSharedPref.getString("Track_10", (new JSONObject()).toString());
