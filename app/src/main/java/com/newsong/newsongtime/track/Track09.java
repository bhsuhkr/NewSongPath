...
public class Track09 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.track_9);

        androidId = Settings.Secure.getString(getContentResolver(), Settings.Secure.ANDROID_ID);

        // initialize map
        rootRef = FirebaseDatabase.getInstance().getReference();
        playersRef = rootRef.child("Track").child(androidId).child("September");
    ...
    public void saveMap(Map<String, Boolean> inputMap) {
        SharedPreferences pSharedPref = getApplicationContext().getSharedPreferences("progress_09", Context.MODE_PRIVATE);
        if (pSharedPref != null) {
            JSONObject jsonObject = new JSONObject(inputMap);
            String jsonString = jsonObject.toString();
            SharedPreferences.Editor editor = pSharedPref.edit();
            editor.remove("Track_09").commit();
            editor.putString("Track_09", jsonString);
            editor.commit();
        }
    }
    ...
    public void loadMap() {
        ...
        try {
            if (pSharedPref != null) {
                String jsonString = pSharedPref.getString("Track_09", (new JSONObject()).toString());
