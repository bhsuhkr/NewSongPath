...
public class Track06 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.track_6);

        androidId = Settings.Secure.getString(getContentResolver(), Settings.Secure.ANDROID_ID);

        // initialize map
        rootRef = FirebaseDatabase.getInstance().getReference();
        playersRef = rootRef.child("Track").child(androidId).child("June");
    ...
    public void saveMap(Map<String, Boolean> inputMap) {
        SharedPreferences pSharedPref = getApplicationContext().getSharedPreferences("progress_06", Context.MODE_PRIVATE);
        if (pSharedPref != null) {
            JSONObject jsonObject = new JSONObject(inputMap);
            String jsonString = jsonObject.toString();
            SharedPreferences.Editor editor = pSharedPref.edit();
            editor.remove("Track_06").commit();
            editor.putString("Track_06", jsonString);
            editor.commit();
        }
    }
    ...
    public void loadMap() {
        ...
        try {
            if (pSharedPref != null) {
                String jsonString = pSharedPref.getString("Track_06", (new JSONObject()).toString());
